...
| [`pay/`](./pay) | 与通用运行时隔离的支付行为 | 有界回环 HTTP；SQLite 沙盒账本；幂等创建；Sandbox Channel；事件绑定的签名 Webhook | 🧪 仅沙盒 |
...
- 拒绝请求体、已知路由上的错误方法和超过 Core 上限的请求头，并限制 Header、请求与 Keep-Alive 超时。
...
- 使用 Shell 协议 v3 校验请求 ID 与运行 ID；在当前服务进程中跨认证连接保留最近 `512` 个已接受请求 ID，把每个运行范围事件关联到唯一运行，拒绝仍在保留范围内的重放与重叠启动，并且只取消匹配的当前运行。
...
- 在配置加载阶段拒绝显式空白的商户 ID 和数据库路径，不打开账本或监听端口；未设置时的默认值及非空白配置保持不变。
- 对重复的 Idempotency-Key 或 webhook 签名请求头返回 400 并关闭连接，不创建对应订单或应用支付事件。
- 与 Core 的通用运行时职责保持隔离。
- 仅接受数字回环地址 `127.0.0.1`，并默认使用端口 `8788`，与 Shell 分离。
- 在所有环境中，启动时拒绝显式为空或仅含空白字符的 webhook 密钥；非空白密钥保持原始内容不变。生产模式仍拒绝默认开发密钥。
- 对请求头和连接生命周期设置边界，请求体上限为 1 MiB，拒绝无请求体路由上的载荷，并返回 no-store/nosniff JSON 响应。
- 每个 Webhook 事件 ID 都绑定其原始类型、Channel 与订单；冲突复用会按失败关闭处理。
- 在业务处理前校验订单和已签名 Webhook 的 JSON 字段类型；格式错误的载荷返回 422，不创建订单或应用支付事件。
- 查单 ID 的 URL 编码无效时返回 400；合法编码的 ID 保持正常查询行为。
- JSON 请求体包含无效 UTF-8 字节时返回 400，不创建订单或应用支付事件；Webhook 仍先对原始字节验签。
- 已知接口收到不支持的方法时返回 405 和 Allow 响应头；未知路径仍返回 404，请求体限制优先处理。
- 请求地址无法解析为 URL 时返回 400 并关闭连接。
- 通道创建成功后，本地保存异常不再把订单误标为失败；保留已记录状态，不自动重新创建支付。未解决的 CREATED 订单恢复仍未实现。

</details>

### 尚未完成

**Navigator 身份、受治理记忆、完整权限执行、Provider 切换后的连续性、设备配对、备份恢复，以及公开的 Ability manifest / sandbox / 签名模型仍在设计或开发中。** Node ID 只是本地安装锚点，不是用户身份或认证凭据。Shell 每个本地连接仍只允许一个活动运行，也不会持久化运行历史。
...
- Shell 请求 ID 与运行 ID 只用于本地协议关联，不是认证、授权、持久身份或权限许可。重放防护有界且仅在当前进程内有效：服务重启时会重置，超过最近 `512` 个已接受请求 ID 的旧条目可能被淘汰。
- Windows 当前依赖用户数据目录继承的 ACL；未来安装器仍需显式配置并验证只有当前用户可访问的 ACL。
- `pay/` 是具有有界 HTTP 资源的回环沙盒，不得处理真实资金。
